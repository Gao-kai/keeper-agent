...
from knowledge.processor.query_process.exception import (
    ValidationError,
    Neo4jError,
    MilvusError,
)
...
from knowledge.utils.bge_m3_embedding_model import (
...
from knowledge.utils.milvus_client import (
    ...
    for one_hop_node in one_hop_nodes:
        ...
        if not item_name:
            relation_texts.append(
                f"{head_entity_name} -({relation})-> {tail_entity_name}"
            )
        else:
            relation_texts.append(
                f"{item_name} {head_entity_name} -({relation})-> {tail_entity_name}"
            )
...
class EntityAligner:
    ...
    def align(
        self, entities: List[str], item_names: List[str], config: QueryConfig
    ) -> EntityAlignResult:
        """
        ...
        hybrid_embeddings: Dict[str, Any] = generate_hybrid_embeddings(
            embedding_model=bge_m3_embedding_model, embedding_docs=entities
        )
        ...
        self._logger.info(
            f"LLM提取出来的用户问题中的实体名称{entities} -> 成功生成BGE-M3向量嵌入模型混合向量 "
        )
    ...
    def align_by_one(
        ...
        for item_name, best_hit_by_item in best_entity_by_item_name.items():
            ...
            results.append(
                {
                    "original_entity_name": entity_name,
                    "aligned_entity_name": best_item_entity_fields.get(
                        "entity_name", ""
                    ),
                    "reason": f"商品 {best_item_entity_fields.get("item_name")} 中最相似的实体名称",
                    "source_chunk_id": best_item_entity_fields.get(
                        "source_chunk_id", ""
                    ),
...
class QueryKnowledgeGraphNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 2. 知识图谱查询编排
        pipeline_result = self.query_graph_pipeline(
            rewritten_query, item_names, query_config, state
        )
    ...
    def validate_inputs(self, state: QueryGraphState):
        ...
        if not item_names or not isinstance(item_names, list):
            raise ValidationError(
                node_name=self.name, message=f"输入参数 [item_names] 校验失败"
            )

        if not rewritten_query or not isinstance(rewritten_query, str):
            raise ValidationError(
                node_name=self.name, message=f"输入参数 [rewritten_query] 校验失败"
            )

        return item_names, rewritten_query

    def query_graph_pipeline(
        self, rewritten_query, item_names, query_config, state
    ) -> QueryGraphPipelineResult:
        """
        知识图谱查询编排主流程。
        ...
        """
        self.log_step(
            step_name="STEP-2", message="开始基于用户问题从LLM中提取抽取实体名称"
        )
        # 1. 用户问题中抽取实体名称
        ...
        self.log_step(
            step_name="STEP-2",
            message=f"基于用户问题从LLM中提取抽取实体名称:{cleaned_entities}",
        )

        # 2. 基于抽取的实体名称和导入时存储在Milvus中的实体名称进行对齐
        entity_aligner = EntityAligner()
        self.log_step(
            step_name="STEP-3", message="基于LLM抽取实体名称在Milvus数据库中进行对齐"
        )
        entity_aligned_result = entity_aligner.align(
            cleaned_entities, item_names, query_config
        )
        self.log_step(
            step_name="STEP-3",
            message=f"基于LLM抽取实体名称对齐结果为：{entity_aligned_result}",
        )
        ...
        self.log_step(
            step_name="STEP-4", message=f"传递给Neo4j中查询的参数对为：{query_pairs}"
        )
        ...
        self.log_step(
            step_name="STEP-5", message=f"查询到的种子节点为：{cleaned_seed_nodes}"
        )
        ...
        self.log_step(
            step_name="STEP-6",
            message=f"查询到的种子节点在其一跳范围内的节点和关系为：{one_hop_nodes}",
        )

        # 6. 对所有种子节点和一跳范围内的所有节点进行加权 种子2 邻居节点1
        nodes_weight_list = neo4j_graph_reader.collect_nodes_by_weight(
            cleaned_seed_nodes, one_hop_nodes
        )
        self.log_step(
            step_name="STEP-7",
            message=f"对种子节点和一跳节点加权后的字典：{nodes_weight_list}",
        )

        # 7. 基于加权后的节点和MENTION_IN关系查询出所有关联的chunk_id
        chunk_nodes_sorted = neo4j_graph_reader.query_chunk_id_by_node_weight(
            nodes_weight_list
        )
        self.log_step(
            step_name="STEP-8",
        ...
        chunk_results = chunk_back_filler.search_chunk_node_in_milvus(
            chunk_nodes_sorted
        )
        self.log_step(
            step_name="STEP-9",
    ...
    def get_neo4j_query_pairs(
        ...
        for aligned_entity_field in aligned_entity_fields:
            ...
            if unique_key not in seen:
                seen.add(unique_key)
                query_pairs.append({"item_name": item_name, "entity_name": entity_name})
...
class Neo4jGraphReader:
    ...
    def __init__(self, config: QueryConfig):
        ...
        self._max_seed_candidates = (
            config.kg_max_seed_candidates
        )  # 每个实体最大种子节点候选数
        self._max_total_seeds = config.kg_max_total_seeds  # 总种子节点上限
        self._max_triples_per_seed = (
            config.kg_max_triples_per_seed
        )  # 每个种子最大三元组数
        self._max_total_triples = (
            config.kg_max_total_triples
        )  # 所有种子节点最大三元组数
        ...
        if self.neo4j_driver is None:
            raise Neo4jError(
                node_name="Neo4jGraphReader查询类", message="Neo4j客户端连接失败"
            )
            ...
            for query_pair in query_pairs:
                ...
                accurate_query_result = self.neo4j_driver.execute_query(
                    query_="""
						...
						""",
                    parameters_={"entity_name": entity_name, "item_name": item_name},
    ...
    def query_one_hop_nodes(
        self, cleaned_seed_nodes: List[Neo4jQueryPair]
    ) -> List[OneHopRelation]:
        """
    ...
    def collect_nodes_by_weight(
        self,
        cleaned_seed_nodes: List[Neo4jQueryPair],
        one_hop_nodes: List[OneHopRelation],
    ) -> List[Dict[str, Any]]:
        """
        ...
        for one_hop_node in one_hop_nodes:
            ...
            if (
                head_entity_name
                and (item_name, head_entity_name) not in nodes_weight_map
            ):
                nodes_weight_map[(item_name, head_entity_name)] = NBR_NODE_WEIGHT

            if (
                tail_entity_name
                and (item_name, tail_entity_name) not in nodes_weight_map
            ):
        ...
        return [
            ...
            for (item_name, entity_name), weight in nodes_weight_map.items()
        ]
...
class ChunkBackFiller:
    def __init__(self, config: QueryConfig):
        ...
        if self.milvus_client is None:
            raise MilvusError(
                node_name="ChunkBackFiller查询类", message="Milvus客户端连接失败"
            )
