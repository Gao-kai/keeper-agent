...
class WebSearchMCPNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        try:
            tool_call_result = asyncio.run(
                self.mcp_server_call(query_config, rewritten_query)
            )
            if tool_call_result:
    ...
    async def mcp_server_call(self, query_config: QueryConfig, rewritten_query: str):
        ...
        search_mcp_server = MCPServerStreamableHttp(
            ...
            params={
                ...
                "headers": {"Authorization": f"Bearer {query_config.openai_api_key}"},
            },
    ...
    def format_tool_call_result(self, tool_call_result: CallToolResult):
        ...
        try:
            ...
            for page in pages:
                ...
                if not snippet:
                    continue
                web_search_docs.append({"title": title, "url": url, "snippet": snippet})
        except Exception as e:
