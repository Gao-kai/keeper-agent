...
class ConfirmItemNameNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 2. LLM提取item_names(获取历史对话上下文供LLM参考获取更加准确商品名)
        chat_history = []
        ...
        extract_result = self.item_name_extractor.extract_by_llm(
            original_query, history_context=chat_history
        )
        ...
        if extracted_item_names:
            ...
            confirmed_item_names, optional_item_names, confirmed_items = (
                self.filter_item_name_by_score_limit(search_result)
            )
        ...
        if confirmed_item_names:
            ...
            if ids_to_update:
                mongo_tool.update_message_item_names(
                    ids=ids_to_update, item_names=confirmed_item_names
                )

        # 7. 将更新后的历史对话消息写入state
    ...
    def vector_search(extracted_item_names: List[str]):
        ...
        hybrid_embeddings_result = generate_hybrid_embeddings(
            embedding_model=bge_m3_model, embedding_docs=extracted_item_names
        )
        # 3. 加载集合
    ...
    def decide(
        ...
        if confirmed_item_names and isinstance(confirmed_item_names, List):
            ...
            # 直接返回 有Answer
            state["answer"] = (
                "抱歉，我无法识别您咨询的具体产品名称，请提供更加准确的产品名称或型号。"
            )

    @staticmethod
    def filter_item_name_by_score_limit(
        ...
        for index, item in enumerate(search_result):
            ...
            # 按照score倒序排列
            sorted_by_score_matches = sorted(
                matches, key=lambda m: m["score"], reverse=True
            )

            # 进行阈值过滤
            high_related_item_names = [
                match
                for match in sorted_by_score_matches
                if match.get("score", 0) >= HIGH_RELATED_SCORE
            ]
        ...
        return (
            confirmed_item_names,
            optional_item_names[:MAX_OPTION_SIZE],
            confirmed_items,
        )

    @staticmethod
    def filter_item_name_by_score_gap(
        ...
        # 将超出最大值差异阈值的直接过滤掉
        return [
            confirmed_item.get("item_name")
            for confirmed_item in confirmed_items
            if max_score - confirmed_item.get("score") <= MAX_SCORE_GAP
        ]
...
class ItemNameExtractor:
    ...
    def extract_by_llm(
        ...
        # 构建提示词模版
        system_message = (
            "你是一个专业的客服助手，擅长从用户问题中做出准确的意图识别和关键信息提取"
        )
        human_message = ITEM_NAME_EXTRACT_TEMPLATE.format(
            history_text=history_context or "暂无历史上下文", query=original_query
        )
    ...
    def parse_llm_response(response_content: str) -> Dict[str, Any]:
        ...
        safe_item_names = (
            [str(item_name).strip() for item_name in raw_item_names]
            if isinstance(raw_item_names, list)
            else []
        )
        raw_rewritten_query = parsed_result.get("rewritten_query")
        safe_rewritten_query = (
            str(raw_rewritten_query).strip()
            if isinstance(raw_rewritten_query, str)
            else ""
        )

        # 返回结果
        return {"item_names": safe_item_names, "rewritten_query": safe_rewritten_query}
